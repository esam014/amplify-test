...
export default function NutritionPlanUpdateForm(props) {
  const {
    id,
    nutritionPlan,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...nutritionPlanRecord };
    setCalorie_target(cleanValues.calorie_target);
    setProtein_target(cleanValues.protein_target);
    setCarb_target(cleanValues.carb_target);
    setFat_target(cleanValues.fat_target);
    setEnd_date(cleanValues.end_date);
    setErrors({});
  };
  const [nutritionPlanRecord, setNutritionPlanRecord] =
    React.useState(nutritionPlan);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(NutritionPlan, id)
        : nutritionPlan;
      setNutritionPlanRecord(record);
    };
    queryData();
  }, [id, nutritionPlan]);
  React.useEffect(resetStateValues, [nutritionPlanRecord]);
  const validations = {
    calorie_target: [],
    protein_target: [],
    carb_target: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            NutritionPlan.copyOf(nutritionPlanRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "NutritionPlanUpdateForm")}
    >
      <TextField
