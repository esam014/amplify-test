...
export default function PlatformConfigUpdateForm(props) {
  const {
    id,
    platformConfig,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...platformConfigRecord };
    setLogo_file(cleanValues.logo_file);
    setFavicon_file(cleanValues.favicon_file);
    setPrimary_color(cleanValues.primary_color);
    setSecondary_color(cleanValues.secondary_color);
    setCustom_url(cleanValues.custom_url);
    setPersonal_web_enabled(cleanValues.personal_web_enabled);
    setPlatform_nutrition_enabled(cleanValues.platform_nutrition_enabled);
    setPlatform_trainer_enabled(cleanValues.platform_trainer_enabled);
    setErrors({});
  };
  const [platformConfigRecord, setPlatformConfigRecord] =
    React.useState(platformConfig);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(PlatformConfig, id)
        : platformConfig;
      setPlatformConfigRecord(record);
    };
    queryData();
  }, [id, platformConfig]);
  React.useEffect(resetStateValues, [platformConfigRecord]);
  const validations = {
    logo_file: [],
    favicon_file: [],
    primary_color: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            PlatformConfig.copyOf(platformConfigRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "PlatformConfigUpdateForm")}
    >
      <TextField
        label="Logo file"
        isRequired={false}
        isReadOnly={false}
        defaultValue={logo_file}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
